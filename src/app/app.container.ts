...
import { MysqlConnection } from './model/mysql.index';
import { PostsSchema } from './model/schemata/posts.schema';
...
export class ApplicaionContainer extends Container {
    ...
    //     // })
    // }
    private bindService(services: (new (args?) => {})[]) {
        services.forEach((service) => {
            this.bind((service.name)).to(service).inSingletonScope();
        });
    }
    private bindComponentsInSingleton(components: (new (args?) => {})[]) {
